...
def run_scraper(difficulty: str, area_text: str, chapter_name: str,
                level: int, question_type: int) -> list[dict]:
    """
    Public API for streamlit_app.py – returns a list of question dicts.
    """
    ...
    try:
        ...
        for q in range(1, total + 1):
            item = _parse_question(driver, test_id, q)
            results.append({
...
def _start_driver() -> webdriver.Chrome:
    # 1) Install and locate chromedriver
    driver_path = chromedriver_autoinstaller.install()
    service = Service(driver_path)

    # 2) Locate installed Chrome/Chromium binary
    import shutil
    chrome_path = (
